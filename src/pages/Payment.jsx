...
const stripeStyle = {
  style: {
    base: { fontSize: "14px", color: "#1f2937", fontFamily: "Inter, sans-serif", "::placeholder": { color: "#9ca3af" } },
    invalid: { color: "#ef4444" },
  },
};


const CheckoutForm = ({ clientSecret, amountInPaise }) => {
  ...
  const allReady = ready.number && ready.expiry && ready.cvc && cardName.trim();

  const handlePay = async () => {
    if (!stripe || !elements || !cardName.trim()) return;
    setPaying(true);
    try {
      ...
      if (error) { toast.error(error.message || "Payment failed"); setPaying(false); return; }
    ...
    <div className="space-y-4">
      <div>
        ...
        <input type="text" value={cardName} onChange={e => setCardName(e.target.value)}
          ...
          className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 transition" />
      </div>
      <div>
        ...
        <div className="w-full px-4 py-3.5 border-2 border-gray-200 rounded-xl focus-within:border-blue-500 transition bg-white flex items-center gap-3">
          ...
          <div className="flex-1">
            <CardNumberElement options={stripeStyle} onChange={e => setReady(p => ({ ...p, number: e.complete }))} />
          </div>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <div className="px-4 py-3.5 border-2 border-gray-200 rounded-xl focus-within:border-blue-500 transition bg-white">
            <CardExpiryElement options={stripeStyle} onChange={e => setReady(p => ({ ...p, expiry: e.complete }))} />
          </div>
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">CVC</label>
          <div className="px-4 py-3.5 border-2 border-gray-200 rounded-xl focus-within:border-blue-500 transition bg-white flex items-center gap-2">
            <div className="flex-1"><CardCvcElement options={stripeStyle} onChange={e => setReady(p => ({ ...p, cvc: e.complete }))} /></div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-xs text-blue-700 flex items-start gap-2">
        ...
        <div><span className="font-semibold">Test card:</span> 4242 4242 4242 4242 · Any future date · Any 3-digit CVC</div>
      </div>
      <button onClick={handlePay} disabled={paying || !stripe || !allReady}
        className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold py-4 rounded-xl transition flex items-center justify-center gap-2 text-base">
        {paying
          ? <><i className="ri-loader-4-line animate-spin" /> Processing...</>
          : <><i className="ri-secure-payment-line" /> Pay ₹{(amountInPaise / 100).toLocaleString("en-IN")}</>}
      </button>
      <p className="text-center text-xs text-gray-400 flex items-center justify-center gap-1">
        <i className="ri-shield-check-line text-green-500" /> Secured by Stripe · SSL Encrypted
      </p>
    </div>
  );
};
...
const Payment = () => {
  ...
  useEffect(() => {
    ...
    if (!currentBooking) {
      toast.error("No active booking found. Please start again.");
      navigate("/");
      return;
    }
    ...
    const bookingId = currentBooking?.booking?.bookingId;
        ...
        <div className="flex-1">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <Elements stripe={stripePromise} options={{ clientSecret }}>
              <CheckoutForm clientSecret={clientSecret} amountInPaise={amountInPaise} />
            </Elements>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            ...
            <div className="space-y-2 text-sm">
              {[
                ...
                { label: "Bus",         value: b?.busName },
                { label: "Route",       value: `${b?.sourceCity} → ${b?.destinationCity}` },
                { label: "Date",        value: formatDate(b?.travelDate) },
                { label: "Time",        value: `${formatTime(b?.departureTime)} → ${formatTime(b?.arrivalTime)}` },
              ...
              <div className="border-t border-gray-100 pt-2 flex justify-between font-bold text-gray-800 text-base">
                <span>Total</span>
                <span className="text-blue-600">₹{Number(b?.totalAmount || 0).toLocaleString("en-IN")}</span>
              </div>
            </div>
          </div>
          ...
          {passengers.length > 0 && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="space-y-2">
                {passengers.map((p, i) => (
                  <div key={p.bookingSeatId || i} className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full font-medium">{p.seatNumber}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-green-50 border border-green-200 rounded-2xl p-4 space-y-2 text-xs text-green-700">
            {[
              ...
              <div key={b.text} className="flex items-center gap-2">
                <i className={b.icon + " text-base"} /> {b.text}
              </div>
            ))}
          </div>
        </div>
